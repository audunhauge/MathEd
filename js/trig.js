...
const tri2svg = p => {
    let scale1 = 10 / p.size.s; // font scale for scale (s:10 = normal)
    // let scale2 = p.size.w / 400;  // font scale for size (w:400 = normal)
    let percent = clamp(100 * scale1, 50, 100);
    let color = p.size.c || "blue";
    let fz = `font-size="${percent}%"`;
    let s = '<text fill="red" x ="30" y="30" >Invalid triangle</text>';
    if (p.valid) {
        s = "";
        if (p.polygon) {
